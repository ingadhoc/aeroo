...
class Aeroo_report(report_sxw):
    ...
    def _insert_html(self, print_id, aeroo_ooo=False):
        def insert_html(data, silent=False):
# convert data
            # try:
            #     unicode(data, "ascii")
            # except UnicodeError:
            #     data2 = unicode(data, "utf-8")
            # else:
            #     # value was valid ASCII data
            #     pass
            # data = preprocess(basic.generate(data).render().decode('utf8').encode(report_xml.charset, 'ignore'))

# html file generation
            from DocumentConverterExtension import DocumentConverter
            import binascii, urllib2
            input_html = NamedTemporaryFile(suffix='.html', prefix='html-file-', delete=False)
            input_html.write(data.encode('utf8'))
            input_html.close()
            os.chmod(input_html.name, 0666)
            path = os.path.abspath(input_html.name)
            #os.chmod(path, 0666)

# Convertion of html file to odt file
            converter = DocumentConverter()
            output_odt = path[:-4]+'odt'
            converter.convert(path, output_odt)
#            output_odt.close()
 #           os.chmod(output_odt, 0666)
 #           os.remove(os.path.abspath(input_html.name))
            os.remove(path)
            output_odt_path = output_odt
# Include_document method
            temp_file = NamedTemporaryFile(suffix='.odt', prefix='aeroo-report-', delete=False)
            if os.path.isfile(output_odt):
                fd = file(output_odt, 'rb')
                output_odt = fd.read()
            else:
                error = False
                try:
                    url_file = urllib2.urlopen(output_odt)
                    output_odt = url_file.read()
                except urllib2.HTTPError, e:
                    os.unlink(temp_file.name)
                    error = _('HTTP Error %s! File not found:') % e.getcode() + ' %s' % output_odt
                except urllib2.URLError, e:
                    os.unlink(temp_file.name)
                    error = _('Error!')+' %s' % e
                except IOError, e:
                    ...
                    try:
                        output_odt = base64.decodestring(output_odt)
                    except binascii.Error:
                        os.unlink(temp_file.name)
                        error = _('Error! File not found:')+' %s' % output_odt
                if error:
                    if not silent:
                        return error
                    else:
                        return None
            try:
                temp_file.write(output_odt)
            finally:
                temp_file.close()
            # os.remove(os.path.abspath(output_odt))                
    ...
    def create_aeroo_report(self, cr, uid, ids, data, report_xml, context=None, output='odt'):
        ...
        oo_parser.localcontext['insert_html'] = self._insert_html(print_id, aeroo_ooo)
        deferred = context.get('deferred_process')
        oo_parser.localcontext['progress_update'] = deferred and deferred.progress_update or (lambda:True)
